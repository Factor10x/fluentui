 ...
 */
export function cleanupDefaultLayerHost(doc: Document, shadowRoot: ShadowRoot | undefined) {
  const host = shadowRoot ? shadowRoot.querySelector(`#${defaultHostId}`) : doc.querySelector(`#${defaultHostId}`);

  if (host) {
    if (shadowRoot) {
      shadowRoot.removeChild(host);
    } else {
